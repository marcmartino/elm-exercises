...
var $author$project$ListOps$appendRec = F2(
	function (listA, listB) {
		...
		while (true) {
			if (listA.b) {
				var a = listA.a;
				var otherA = listA.b;
				var $temp$listA = otherA,
					$temp$listB = A2($elm$core$List$cons, a, listB);
				listA = $temp$listA;
				listB = $temp$listB;
				continue appendRec;
			} else {
...
var $author$project$ListOps$reverse = function (list) {
	if (list.b) {
		...
		var xs = list.b;
		return _Utils_ap(
			$author$project$ListOps$reverse(xs),
			_List_fromArray(
				[x]));
	} else {
		return _List_Nil;
...
var $author$project$ListOps$append = function (xs) {
	return $author$project$ListOps$appendRec(
		$author$project$ListOps$reverse(xs));
};
...
var $author$project$ListOps$foldr = F2(
	function (f, acc) {
		return A2(
			$elm$core$Basics$composeR,
			$author$project$ListOps$reverse,
			A2($author$project$ListOps$foldl, f, acc));
	});
var $author$project$ListOps$concat = A2($author$project$ListOps$foldr, $elm$core$Basics$append, _List_Nil);
			...
			if (list.b) {
				...
				if (f(x)) {
					return A2(
						$elm$core$List$cons,
						x,
						A2($author$project$ListOps$filter, f, xs));
				} else {
					var $temp$f = f,
						$temp$list = xs;
					f = $temp$f;
...
var $author$project$ListOps$length = function (list) {
	if (list.b) {
		var rest = list.b;
		return 1 + $author$project$ListOps$length(rest);
	} else {
		return 0;
	}
};
	...
	function (f, list) {
		if (list.b) {
			...
			return A2(
				$elm$core$List$cons,
				f(x),
				A2($author$project$ListOps$map, f, xs));
		} else {
			return _List_Nil;
		}
	});
...
var $author$project$Test$Generated$Main3424361298$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/home/marmar/exercism/elm/list-ops/tests/Tests.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 140582317243819
	},
...
_Platform_export({'Test':{'Generated':{'Main3424361298':{'init':$author$project$Test$Generated$Main3424361298$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-22694.sock";
